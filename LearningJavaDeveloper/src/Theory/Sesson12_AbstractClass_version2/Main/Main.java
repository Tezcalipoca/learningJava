...
//        playWithShapes();
    ...
    public static void playWithShapes() {
        ...
        r2.paint(); // paint của ai?? -> CHA - Shape, khi chạy chạy của con vì @Override (qua mặt)
        c1.paint(); // paint của ai?? -> CON - Rectangle, Khi chạy chạy của con
        c2.paint(); // paint của ai?? -> CHA - Shape, khi chạy chạy của con vì @Override (qua mặt)
...
//        r2.paint(); // Khai cha, new con, paint() của Shape, Chạy của Rect (qua mặt Shape)
//
//        s1.paint(); // Khai con, new con, chạy paint() của Square
//        s2.paint(); // Khai cha, new con, paint() của Rect, Chạy của Square (qua mặt Rect)
//        s3.paint(); // Khai ông nội, new con, paint() của Shape, Chạy của Square (qua mặt Shape)
//
//        c1.paint(); // Khai con, new con, chạy paint() của Circle
//        c2.paint(); // Khai cha, new con, paint() của Shape, Chạy của Circle (qua mặt Shape)
//
//        t1.paint(); // Khai con, new con, chạy paint() của Triangle
//        t2.paint(); // Khai cha, new con, paint() của Shape, Chạy của Triangle (qua mặt Shape)
...
//        Shape arr[] = new Shape[] {new Rectangle(...), new Square(...), new Circle(...)}
